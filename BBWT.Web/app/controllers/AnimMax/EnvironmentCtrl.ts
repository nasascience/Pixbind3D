...
module Controllers {
    ...
    export interface IEnvironmentCtrlScope extends ng.IScope {
        AnimPage: () => void;
        CharPage: () => void;
        EnvironmentVM: any;
        ...
        EnvironmentsSource: kendo.data.DataSource; 
        FilterCheck: (e) => void;
        Envfilters: any;
    }
    ...
    export class EnvironmentCtrl {
        ...
        constructor(
            ...
            $scope.EnvironmentsSource = new kendo.data.DataSource({
                transport: {
                    read: {
                        url: "api/Products/GetAllProducts?Type=Environment",
                        dataType: "json"
                    }
                },
                pageSize: 10
            ...
            $http.get('api/Products/GetCategories?type=Environment')
                .success((data) => {
                    $scope.Envfilters = data;
                    $scope.Envfilters.forEach((item) => {
                        console.log(item.Name);
                        filterConditions.push({ field: 'Category', operator: 'eq', value: item.Name });
                    });

                    //It has to be under the ProjectsSource Variable in order to make it work
                    $scope.EnvironmentsSource.filter([
                        {
                            "logic": "or",
